...
export class Surface {
  ...
  /** Какое это полотно. Спрашивает выбор реестра предметов: решение зависит
   *  от того, влезает ли в кадр именно МОНИТОР — ноутбук уже и в кадр
   *  помещается всюду. Искать монитор по габаритам, как это делает замер,
   *  здесь нельзя: порядок обхода сцены не обещан, а ошибиться молча легко. */
  get screen() {
    return this.state.screen
  }

  /** Высота холста. Нужна выбору реестра, чтобы перевести экранный размер
   *  полотна в кегль: масштаб текста в `paint.ts` — это `k = H / 870`.
   *  Спрашивается у самого полотна, а не вписывается числом на стороне
   *  вызывающего: число задаётся здесь, при сборке экранов. */
  get canvasHeight() {
