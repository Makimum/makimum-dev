...
export interface HotspotSpin {
  ...
  dragging(): boolean
  /** Толчок без указателя — с клавиатуры. У нажатия нет ни стороны, ни
   *  скорости, поэтому предмет решает сам, что значит «толкнули». */
  nudge(): void
  label(): string
}
...
/*
 * Здесь стоял `MOBILE_HOTSPOTS` — тот же фильтр по `kind`, но выбиравшийся
 * по `pointer: coarse`. Он и был причиной того, что на ЛЮБОМ iPad комната
 * отдавала 2 предмета из 6: Safari на iPadOS сообщает грубый указатель и с
 * пристёгнутой клавиатурой.
 *
 * Правило переехало в `pickHotspots()` (`src/lobby.ts`) и решается теперь не
 * тем, чем целятся, а тем, читаются ли документы на этом экране —
 * см. `monitorReadable()` в `main.ts`.
 */
